...
namespace AI_AI_Agent.Infrastructure.Orchestration.Stores;
...
public sealed class InMemoryRunStore : IRunStore
{
    ...
    { var state = (DateTime.UtcNow, (DateTime?)null); _runs[runId] = state; return Task.FromResult(state); }
    public Task MarkRunEndAsync(Guid runId, DateTime ended)
    { _runs.AddOrUpdate(runId, _ => (DateTime.UtcNow, ended), (_, s) => (s.started, ended)); return Task.CompletedTask; }
    public Task MarkStepAsync(Guid runId, string stepId, StepState state)
    { _steps[(runId, stepId)] = state; return Task.CompletedTask; }
}
