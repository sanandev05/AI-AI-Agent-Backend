...
namespace AI_AI_Agent.Application.Budgets;
...
public sealed class BudgetManager : IBudgetManager
{
    ...
    private sealed class Scope : IDisposable
    {
        ...
        {
            _owner = owner; _runId = runId; _stepId = stepId;
        }
        public void Dispose()
        {
            if (_disposed) return; _disposed = true;
            _owner._cts.TryRemove((_runId, _stepId), out var cts); cts?.Dispose();
        }
    }

    public ValueTask DisposeAsync()
    {
        foreach (var kvp in _cts) { kvp.Value.Cancel(); kvp.Value.Dispose(); }
        _cts.Clear();
